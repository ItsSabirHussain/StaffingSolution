...
class ManagerLogin extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      email: "manager@staffingsolution.com",
      password: "password",
    };
  ...
  doLogin(e) {
    e.preventDefault();
    localStorage.setItem("staff", "authenticated");
  ...
  render() {
    const { from } = this.props.location.state || {
      from: { pathname: "/manager" },
    };

    if (
      ManagerLogin.isAuthenticated(
      ...
      <div className="auth-page">
        <Container>
          <Widget
            ...
            title={<h3 className="mt-0">Manager Login</h3>}
          >
